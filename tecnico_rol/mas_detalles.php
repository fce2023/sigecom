...
foreach (array_slice($detalleTecnicoProducto2, $startIndex, $limit) as $detalle2) {
    echo "<tr>
            ...
            <td>
                <button class='btn btn-danger btn-sm' onclick='eliminarMaterial({$detalle2['id_producto']}, \"{$id_detalle_tecnico}\")'>
...
function eliminarMaterial(idProducto) {
    ...
    if (confirmar) {
        ...
        const url = `eliminar_material.php?id_detalle_tecnico=<?php echo $id_detalle_tecnico; ?>&id_producto=${idProducto}`;
        window.location.href = url;
    }
}
